...
5. Avvia i due processi in sviluppo (in terminali separati):
   ...
   ```bash
   npm run dev      # Next.js — http://localhost:4200
   npm run worker   # worker pg-boss (processing PDF, summarization)
   ```

6. Accedi su [http://localhost:4200](http://localhost:4200) con le credenziali del seed.
...
4. Fa doppio click su `avvia.bat` e aspetta (la primissima volta scarica anche il modello AI locale per l'indicizzazione dei PDF, può richiedere qualche minuto): al termine si apre da solo il browser su `http://localhost:4200`.
